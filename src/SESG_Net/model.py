...
class SegmentSpatialTemporalStratModel(nn.Module):
    def __init__(
        ...
        use_stratigraphy: bool = True,
    ) -> None:
        super().__init__()
        self.use_sequence = use_sequence
        self.use_spatial = use_spatial
        self.use_stratigraphy = use_stratigraphy
        if use_sequence:
        ...
        if use_spatial:
            ...
            self.formation_embedding = nn.Embedding(num_formations, formation_embedding_dim)
            ...
            self.graph_sage = DynamicWeightedGraphSAGE(spatial_dim, spatial_dim, dropout)
            self.final_neighbor_k = min(final_neighbor_k, candidate_neighbors.shape[1])

        classifier_input = 0
        if use_sequence:
            classifier_input += self.sequence_encoder.out_channels
            if self.missing_branch.enabled:
                classifier_input += self.missing_branch.out_dim
        if use_spatial:
            classifier_input += spatial_dim
    ...
    def _encode_supernodes(self, node_ids: torch.Tensor) -> torch.Tensor:
        ...
        if not self.use_stratigraphy:
            group = torch.zeros_like(group)
            formation = torch.zeros_like(formation)
    ...
    def forward(
        ...
        if self.use_sequence:
            fused.append(self.sequence_pool(self.sequence_encoder(sequence)))
            missing_features = self.missing_branch(missing)
            if missing_features is not None:
                fused.append(missing_features)
        if self.use_spatial:
            fused.append(self.encode_spatial_batch(segment_ids))
