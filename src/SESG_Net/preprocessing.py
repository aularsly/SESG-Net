...
def build_candidate_graph(
    ...
    well_table = (
        segments[["WELL_ID", "X_LOC", "Y_LOC", "TRAIN_POINTS"]]
        .groupby("WELL_ID", as_index=False)
        .agg({"X_LOC": "median", "Y_LOC": "median", "TRAIN_POINTS": "sum"})
    )
    source_wells = well_table[well_table["TRAIN_POINTS"] > 0]
    source_xy = source_wells[["X_LOC", "Y_LOC"]].to_numpy(dtype=np.float32)
    source_ids = source_wells["WELL_ID"].to_numpy(dtype=np.int32)

    well_candidates: Dict[int, List[Tuple[int, float]]] = {}
    for _, row in well_table.iterrows():
        well_id = int(row["WELL_ID"])
        if not len(source_ids):
            well_candidates[well_id] = []
            continue
        distance = np.linalg.norm(source_xy - np.array([row["X_LOC"], row["Y_LOC"]]), axis=1)
        candidates = [
            (int(source_ids[index]), float(distance[index]))
            for index in np.argsort(distance)
            if int(source_ids[index]) != well_id and float(distance[index]) <= radius
        ]
        well_candidates[well_id] = candidates[: config.candidate_well_k]

    by_well_stratum: Dict[Tuple[int, int, int], Dict[str, np.ndarray]] = {}
    group_keys = ["WELL_ID", "GROUP_ID_MODE", "FORMATION_ID_MODE"]
    for key, subset in segments.groupby(group_keys, sort=False):
        ordered = subset.sort_values("Z_LOC")
        by_well_stratum[tuple(map(int, key))] = {
            "ids": ordered["SEGMENT_ID"].to_numpy(dtype=np.int32),
            "z": ordered["Z_LOC"].to_numpy(dtype=np.float32),
        }
    ...
    for source in range(node_count):
        ...
        for neighbor_well, horizontal_distance in well_candidates.get(int(well_ids[source]), []):
            key = (neighbor_well, int(group_ids[source]), int(formation_ids[source]))
            stratum = by_well_stratum.get(key)
            if stratum is None:
                continue
            positions = _nearest_positions(
                stratum["z"],
                float(z[source]),
                config.candidate_per_well,
            )
            for position in positions:
                target = int(stratum["ids"][position])
                vertical_distance = abs(float(z[source] - z[target]))
                distance_3d = float(np.hypot(horizontal_distance, vertical_distance))
                ...
                relations.append((preliminary_score, target, relation))
